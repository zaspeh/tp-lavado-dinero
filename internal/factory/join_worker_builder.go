...
func buildJoinWorker[T, V, R any](cfg joinWorkerConfig[T, V, R]) (workers.Worker, error) {
	...
	newCoordinator := coordinator.NewAloneCoordinator(cfg.ID)
	...
	dynSender := sender.NewDynamicKeySender[V, R](
		resultExchange,
	...
	eng := engine.NewStatefulEngine(recv, dynSender, cfg.Processor, newCoordinator, cm)
